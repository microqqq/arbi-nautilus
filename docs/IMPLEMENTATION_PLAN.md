...
- 本轮离线收尾：使用已资格安装包，在独立临时目录测量CID的1千/1万/10万条合成绑定历史，各5次新增写入和3次冷读，报告文件大小、耗时、事件循环单次停顿。仅bindings、不含费用历史，为成本下界；不据此认证完整Maker/EA容量或场馆fill-to-hedge p99，不改存储或删除去重历史。随后按真实指标记录运行边界和剩余项。
